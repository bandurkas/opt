...
WINNER = {
    ...
    "sigma": 0.6,
    "spread_pct": 2.5, # Realistic 2.5% slippage
}

print("Fetching 45 days of data...", flush=True)
data = fetch_set("ETHUSDT", days=45, intervals=("5", "15", "60"))
...
for s in sims:
    opt = s.get("option", {})
    if "pnl_pct" in opt:
        # Approximate Bybit fee drag: ~2% of premium round-trip
        pnls.append(opt["pnl_pct"] - 2.0)

if pnls:
    wins = [p for p in pnls if p > 0]
    print(f"=== 45-DAY VERIFICATION ===")
    ...
    print("No trades generated in 45 days.")
