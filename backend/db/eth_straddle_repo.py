"""ETH straddle repository — eth_straddle_positions, eth_straddle_equity_snapshots,
eth_straddle_state. Same function surface as paper_repo.py (by design — reconcile.py
and live_safety wiring are written against that surface, so this module is a
drop-in for the ETH straddle loop without any bespoke glue)."""
...
def ensure_state(start_equity_usd: float) -> dict:
    """Get or create the singleton eth_straddle_state row."""
    conn = get_conn()
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT * FROM eth_straddle_state WHERE id = 1")
            row = cur.fetchone()
            if row:
                return dict(row)
            ...
            cur.execute(
                """
                INSERT INTO eth_straddle_state (id, started_at_ms, start_equity_usd)
                VALUES (1, %s, %s)
                RETURNING *
                """,
...
def update_state(*, last_cycle_id: int | None = None,
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"UPDATE eth_straddle_state SET {', '.join(fields)} WHERE id = 1",
                values,
            )
        conn.commit()
    finally:
...
def open_position(*, cycle_id: int, leg: str, opened_at_ms: int,
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO eth_straddle_positions (
...
def close_position(position_id: int, *, closed_at_ms: int,
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                UPDATE eth_straddle_positions SET
                  status = %s,
                  closed_at_ms = %s,
                  exit_debit_usd = %s,
                  pnl_pct = %s,
...
def open_positions() -> list[dict]:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT * FROM eth_straddle_positions
                 WHERE status = 'open'
...
def recent_positions(limit: int = 50) -> list[dict]:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM eth_straddle_positions ORDER BY opened_at_ms DESC LIMIT %s",
                (limit,),
            )
            return [dict(r) for r in cur.fetchall()]
    finally:
...
def insert_equity_snapshot(*, ts_ms: int, equity_usd: float,
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO eth_straddle_equity_snapshots
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                ...
                SELECT ts_ms, equity_usd, realized_usd, unrealized_usd, n_open, n_closed, max_dd_pct
                  FROM eth_straddle_equity_snapshots
                 WHERE ts_ms >= %s
                 ORDER BY ts_ms ASC
                """,
                (cutoff,),
...
def latest_equity() -> dict | None:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM eth_straddle_equity_snapshots ORDER BY ts_ms DESC LIMIT 1"
            )
            row = cur.fetchone()
            return dict(row) if row else None
    finally:
...
def peak_equity_since(ts_ms_floor: int) -> float | None:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT MAX(equity_usd) FROM eth_straddle_equity_snapshots WHERE ts_ms >= %s",
                (ts_ms_floor,),
            )
            row = cur.fetchone()
...
def realized_pnl_since(ts_ms: int) -> float:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT COALESCE(SUM(pnl_usd), 0) FROM eth_straddle_positions "
