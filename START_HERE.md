...
> **Последнее обновление:** 2026-06-16 · HEAD `main` `46d2c92`+ · local = GitHub = VPS, дерево чистое.
...
- **Недавно сделано (2026-06-16):** внедрён+задеплоен tail-risk кэп `MAX_OPEN_POSITIONS=4`;
  устранён старый deploy-landmine (чистая пересборка); почищен Docker (~5GB); собран план.
...
Из `ROADMAP.md §3/§4`, на выбор (решение — за пользователем):
- **Фаза A1 — CB race** (`FUTURE_WORK §5.2`): самый важный для достоверности гейта, чистый
  unit-тест, без риска для edge. Рекомендуемый первый шаг.
